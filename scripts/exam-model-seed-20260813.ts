...
const 模块账: Record<string, Record<string, unknown>> = {
  绝对值: {
    ...
    来源册:
      "举一反三产物/打卡/七上绝对值压轴突破（10 天 × 12 题 = 120 题，在售，网盘 4nmg）",
  ...
  整式: {
    ...
    dsl: 整式qbank,
    出题能力:
      "可参数化重出（class E 表达式树 + 5 个题目构造器 q/qv/qab/qbr/qm）",
    ...
    复用方:
      "订阅特训/_产线/群卷_整式{,2,_批量}.py（另加 4 组数值代入双录 448 次比对）",
  },
  混合七上: {
    ...
    验算: "verify(items) = 逐行恒等（sympy 双路）+ 题面不撞 + 值域闸 + 步数闸 + 考点配额断言；kp_report() 出考点分布",
    考点码正本:
    ...
    复用方:
      "订阅特训/_产线/群卷_混合_批量.py（seed 由「群打卡-第1期-第N天」CRC32 定）、明日卷.py",
  },
  实数: {
    模块: "实数计算 · 表达式 DSL",
    dsl: 实数qbank,
...
const 首铺表: 首铺条目[] = [
  // ── 绝对值线（清单 1~6）───────────────────────────────────────────────────
  {
    ...
    verdictNote:
      "首铺批量转正（人主导）：DSL 在盘可跑、参数域明确、chk() 有独立验算路。",
  },
  {
    序: 2,
    模块: "绝对值",
    ...
    varSpec: {
      模板函数: 4,
      参数: "T2_min(m,c,k,front,var) / T2_max(m,c,k,var) / T2_revmin(m,c,var) / T2_revmax(m,c,var)",
    ...
    verdictNote:
      "首铺批量转正（人主导）：与已有模型卡 TY27 同源，跨期去重键明确。",
  },
  {
    序: 3,
    模块: "绝对值",
    ...
    verdictNote:
      "首铺批量转正（人主导）：模型卡在盘，e 须偶数这条参数约束已进 var_spec。",
  },
  {
    序: 4,
    模块: "绝对值",
    ...
    verdictNote:
      "首铺批量转正（人主导）：独立验算走分段断点求极值，第二条路是真的另一条路。",
  },
  {
    序: 5,
    模块: "绝对值",
    ...
    kpRef: "绝对值中的分类讨论",
    kpRefWhy:
      "预估「绝对值的分类讨论 / 不重不漏」→ 正名 绝对值中的分类讨论（其别名含「分类讨论思想」）",
  ...
  },
  {
    ...
    kpRef: "符号商 |a|/a 型的计算",
    kpRefWhy:
      "预估「符号商 / 多变量符号穷举」→ 正名 符号商 |a|/a 型的计算（后半句已补成别名，见 ALIASES）",
    ...
    verdictNote:
      "首铺批量转正（人主导）：题面与求值同源自 _TERMS，题面改了答案跟着改，不会脱节。",
  },
  ...
  // ── 整式线（清单 8~12）──────────────────────────────────────────────────
  {
    ...
    kpRef: "合并同类项",
    kpRefWhy:
      "预估「合并同类项法则」→ 正名 合并同类项（预估名已补成别名，见 ALIASES）",
    dslRef: `${整式qbank}#q`,
    difficulty: 1,
    varSpec: {
      构造器: "q(stem, expected) 纯化简",
      梯度: "两组同类项(一组全抵消) → 三项+两项 → 带常数项 → 三字母(一组抵消) → a²/ab/b² 二次型 → 二次三项式",
      配额: "6 题/天",
      题型: "化简",
      节: "七上整式的加减打卡 · 节①",
    },
    note: "整式线节①，梯度六级已定。",
    verdictNote:
      "首铺批量转正（人主导）：expected 与表达式树双录比对，出题即验算。",
  },
  {
    序: 9,
    模块: "整式",
    ...
    verdictNote:
      "首铺批量转正（人主导）：「答案是 0」「答案是纯常数」两道保留题是这一族的错因靶子。",
  },
  {
    序: 10,
    模块: "整式",
    ...
    kpRef: "整式的化简求值",
    kpRefWhy:
      "预估「整式化简求值 / 整体代入」→ 正名 整式的化简求值（其别名含「化简求值」「整式化简求值」）",
    dslRef: `${整式qbank}#qv`,
    difficulty: 2,
    varSpec: {
      ...
      模型卡:
        "举一反三产物/解题模型库/整式化简求值十模型.md（一族十子 = 十天）",
    },
    note: "整式线型②。模型卡「十模型」一族十子，正好铺十天。",
    verdictNote:
      "首铺批量转正（人主导）：subs 用 Rational 不用 float，代入值不会有浮点毛刺。",
  },
  {
    序: 11,
    模块: "整式",
    ...
    kpRef: "整式的和差与整体加括号",
    kpRefWhy:
      "预估「整式的加减运算」偏泛；正名取 整式的和差与整体加括号（其别名含「A±B 型」「和与差」）",
    dslRef: `${整式qbank}#qab`,
    difficulty: 3,
    varSpec: {
      构造器:
        "qab(defs, ask, expected)：defs = {'A': 树, 'B': 树}，ask ∈ {'A+B','A-B','2A-B',…}",
      题型: "解答",
      节: "七上整式的加减打卡 · 型③",
    },
  ...
  },
  {
    ...
    verdictNote:
      "首铺批量转正（人主导）：考点从预估的概念条改挂到专门条，理由已进 kpRefWhy。",
  },
  ...
  },
  {
    ...
    kpRef: "绝对值的化简与去号",
    kpRefWhy:
      "err 码 abs → 正名 绝对值的化简与去号（其别名含 abs / 绝对值脱号 / 去绝对值）",
  ...
  },
  {
    ...
    kpRef: "有理数乘方的运算与符号判定",
    kpRefWhy:
      "err 码 pow → 正名 有理数乘方的运算与符号判定（其别名含 pow / 乘方定号 / 幂的符号）",
    ...
    verdictNote:
      "首铺批量转正（人主导）：对照对生成器的缺口如实挂账，不因为「有缺口」就不登记现有能力。",
  },
  {
    序: 17,
    模块: "混合七上",
    ...
    kpRef: "有理数运算的简便技巧",
    kpRefWhy:
      "err 码 dist → 正名 有理数运算的简便技巧（其别名含 dist / 分配律逆用 / 提公因数 / 运算律简算）",
    ...
    varSpec: {
      ...
      生成器:
        "gen_dist_forward / gen_dist_reverse / gen_div_by_unit_frac（3 个）",
      配额: "默认 3/卷",
      题型: "计算",
    },
  ...
  },
  {
    ...
    kpRef: "分数与小数混合的有理数运算",
    kpRefWhy:
      "err 码 fracdec → 正名 分数与小数混合的有理数运算（其别名含 fracdec / 分小互化 / 分数小数）",
  ...
  },
  {
    ...
    kpRef: "有理数的混合运算",
    kpRefWhy:
      "err 码 order → 正名 有理数的混合运算（其别名含 order / 四则混合运算 / 脱式计算）",
    ...
    verdictNote:
      "首铺批量转正（人主导）：硬顶如实进 var_spec，将来断供时查得到原因。",
  },
  {
    序: 20,
    模块: "混合七上",
    ...
    kpRef: "多重括号的有理数运算",
    kpRefWhy:
      "err 码 paren → 正名 多重括号的有理数运算（其别名含 paren / 多重括号 / 括号嵌套）",
  ...
  // ── 实数线（清单 27~30）🔴 无参数化生成器 ───────────────────────────────
  {
    ...
    kpRef: "开方直取与根式化简",
    kpRefWhy:
      "预估「平方根 / 立方根 / 算术平方根」是三个概念条；本族是**动作**，正名 = 开方直取与根式化简（别名含「开方直取」）",
  ...
  },
  {
    ...
    kpRef: "用平方根立方根的意义解方程",
    kpRefWhy:
      "预估「用平方根/立方根的意义解方程」即正名（别名含「开方解方程」「x²=a 型」）",
...
async function 铺(): Promise<number> {
  ...
  for (const e of 首铺表) {
    if (已有.has(e.name)) {
      跳 += 1;
      say(
        `  · [${String(e.序).padStart(2)}] ${e.name}：库里已有同名模型，跳过（幂等）`,
      );
      continue;
    }
    try {
  ...
  say(细);
  say(
    `🔴 归不了位、如实跳过的 ${SKIPPED.length} 条（备料候选 26 − 可归位 ${首铺表.length}）：`,
  );
