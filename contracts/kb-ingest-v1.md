...
🔴 `figures` 是 punch-ingest/v1 **没有的面**（那边只有册级宣发图、走目录约定），是 kb-ingest/v1
作为其超集**新增**的题级配图通道 —— 见 §3 映射表末尾与 §3.1 第 2 条。
...
### 2.4 `［图·内联SVG］` 占位符约定（源图不是文件时）

有一类源**根本没有图片文件**：专项卷是 Python 生成器（`gen_*.py`）吐的 HTML，图是
函数现画的**内联 `<svg>`**，磁盘上找不到对应的 `.png`。抽取器把这种图位原样转写成一个
固定占位串：

```
［图·内联SVG］          ← 全角方括号，中间是「图」+ 间隔号 + 「内联SVG」
```

🔴 **它是一面红旗，不是一张图**。约定是这样闭合的：
