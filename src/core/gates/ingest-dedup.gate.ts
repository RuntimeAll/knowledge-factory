 ...
 *                      🔴 只认**标签形状** `</?[a-zA-Z][^<>]*>`，数学裸不等号
 *                      （`a < 0，b < 0，c > 0`）一个字都不许吃 —— 见 {@link RE_TAG}。
 ...
 * ① HTML 标签与常见实体
 *
 * ✅ **已修（2026-08-13 · 003-E5）——「宽松剥法吃数学不等号」的旧账已了**。
 *
 *   曾经这里写的是 `<[^>]*>`：**宽松**剥法，会把「一个数学 `<` 到下一个 `>`
 *   之间的整段」当成标签吃掉（G-3 核查员点名的方法论坑①）。003-E4 在真库 60 行里
 *   实测到 **2 行**中招：
 *     · `q_01KZVF46KBKBTWEAYK207HGTZA`：`有理数 a < 0，b < 0，c > 0，且…`
 *        归一后变成 `有理数a0且…`（`0，b < 0，c` 整段被吃）；
 *     · `q_01KZVFBGNZZCZY86M9YXFRHHYS`：`（用 < 或 > 或 = 号填空）` → `(用或=号填空)`。
 *   后果是**查重键失真**：两道只在被吃掉那一段上有区别的题会撞成同一道
 *   （假 `DUPLICATE` 红灯）。正本 `question.stem` 从来一个字没少，错的只是这个 hash。
 *
 *   003-E5 按总指挥裁决换成 `fts.ts` 的 {@link stripHtmlForSeg} 同款**严格形状**
 *   `</?[a-zA-Z][^<>]*>`：`<` 或 `</` 后必须**紧跟** ASCII 字母（标签名不可能以空格/
 *   数字/中文开头），且属性区里**不许再出现 `<`**（合法 HTML 里属性内的 `<` 一律写作
 *   `&lt;`；不加这条，`b<c … >` 那种跨越式误吞还在）。两条归一路（查重键 / 分词喂料）
 *   自此同一把尺子。
 *
 *   连带做掉的两件事（换口径 = 全库 match_key 作废重算，不能只改代码）：
 *     · 撞键预检：新口径下 60 行两两不撞（预检撞 = 真重复题，那要停手报告，不是硬写）；
 *     · 一次性重算：`scripts/recompute-matchkey-20260813.ts`，
 *       审计 `tool='matchkey_recompute_20260813'`，实际改动 2 行（就是上面两条）。
 */
const RE_TAG = /<\/?[a-zA-Z][^<>]*>/g;
/**
 * 🔴 顺序钉死：**先剥标签、后解实体**（与 `fts.ts` 同口径）。
 *    反过来的话 `&lt;span&gt;`（一段被转义、本该当正文读的文本）会先还原成
 *    `<span>`，再被当标签剥掉 —— 凭空吃掉正文。
 * 🔴 `&amp;` 必须排最后：先解它的话 `&amp;lt;` 会变成 `&lt;` 再被解成 `<`（二次解码）。
 */
