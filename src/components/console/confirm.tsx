 ...
 *   ③ Popconfirm（少数几处）。**server action 表单这一路**统一成 ③：
 *   **确认页照旧留着**（破坏性动作的定式不改，PRD-003-D 立的规矩），
 *   但最后那一下一律再弹一层，把「按下去会动哪些行」写进弹层。
 *
 * 🔴🔴 2026-08-15 更正适用范围：这段原来写的是「（全站）现在统一成 ③」——
 *   而 `app/grading/intake/form.tsx` 里同时写着「全站写操作一律 Modal」，
 *   两处互相矛盾地各自宣称自己是「统一」的（验收判红项）。
 *   口径的**唯一正本**在 `src/app/write-ops.ts`：分界线是**提交机制** ——
 *     · `<form action={serverAction}>` 的表单 → 本组件（Popconfirm + requestSubmit
 *       + useFormStatus 防连点），14 处；
 *     · client 侧 fetch 提交、影响面要现算成一段 JSX 的 → Modal（收卷录入、
 *       终审确认/打回/撤回），4 处。
 *   两种都「列明影响面」，检查单那条的实质要求是满足的；要不要收成一种由用户拍板
 *   （write-ops.ts 的 `形态待拍板`）。**第三种「没有确认层」是缺陷**，已在
 *   /cause/map 收编完毕，`tests/write-whitelist.test.ts` 盯着不许再出现。
 *
